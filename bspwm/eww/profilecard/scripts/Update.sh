...
if [ -z "$packages" ]; then
    selected_packages=$(echo -e "Exit" | rofi -dmenu -i -markup-rows -matching fuzzy -p "No packages to update:" -theme "$HOME"/.config/bspwm/src/rofi-themes/Update.rasi -dpi 1)
else
    update_all_option="Update all"
    selected_packages=$(echo -e "$update_all_option\n${packages}" | rofi -dmenu -i -multi-select -markup-rows -matching fuzzy -p "Select packages to update:" -theme "$HOME"/.config/bspwm/src/rofi-themes/Update.rasi -dpi 1)
fi
